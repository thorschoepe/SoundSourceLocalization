...
def histogram_overlay(spiketimes,populations_size, simtime, x_labels, y_labels, path_save, x_lim):
	...
	for num in range(1, number_subplots+1):
		...
		#ax.set_xlim(x_lim)
		ax.hist(np.array(spikes).T.flatten(), simtime/300, density=False, alpha = 0.5)
...
def histogram_singleneuron_overlay(spiketimes,populations_size, simtime, x_labels, y_labels, path_save, x_lim):
	...
	for num in range(1, number_data+1):
		...
		for i in range(len(spiketimes[num-1])):
			...
			ax.hist(spikes[i], simtime/300, density=False, alpha = 0.5, histtype = 'step', color = colors[(num-1)*4 +i] )
		ax.set_ylabel(y_labels[num-1])
		ax.set_xlabel(x_labels)
		#ax.set_xlim(x_lim)
...
simtime = 12000000 # Simulation time [ms]
timestep = 1
...
class Populations:
	# ring attractor
	spike_source = p.Population(1, p.SpikeSourcePoisson, {'rate':100, 'start':0, 'duration':2000}, label = "input_pop")
...
data = [Spikes.p_en_right, Spikes.p_en_left, Spikes.e_pg_right, Spikes.e_pg_left, Spikes.tde_in, Spikes.out_r, Spikes.out_l, Spikes.onset, Spikes.gi]
np.save('/home/spinnaker/Documents/projects/Sevilla_2021_TDE_SSL/data/final/closed_loop/' + str(frequency) + 'Hz/sweep_frequ' + str(frequency) +'_datetime_'+ str(datenow)+'.npy',data)

	
### plots ########################################################################################################################################################

spikedata = [Spikes.p_en_right, Spikes.p_en_left, Spikes.e_pg_right,Spikes.e_pg_left,Spikes.tde_in, Spikes.out_r, Spikes.out_l, Spikes.onset]
...
for i in range(num_channels):
	Spikes.out_r_2 = Spikes.out_r.segments[0].spiketrains[i]
	Spikes.out_l_2 = Spikes.out_l.segments[0].spiketrains[i]
