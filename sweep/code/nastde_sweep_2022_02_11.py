...
def histogram_singleneuron_overlay(spiketimes,populations_size, simtime, x_labels, y_labels, path_save, x_lim):
	...
	for num in range(1, number_data+1):
		...
		for i in range(len(spiketimes[num-1])):
			...
			ax.hist(spikes[i], simtime/1000, density=False, alpha = 0.5, histtype = 'step', color = colors[(num-1)*4 +i] )
		ax.set_ylabel(y_labels[num-1])
		ax.set_xlabel(x_labels)
		#ax.set_xlim(x_lim)
...
data = [Spikes.p_en_right, Spikes.p_en_left, Spikes.e_pg_right, Spikes.e_pg_left, Spikes.tde_in, Spikes.angle_difference_r, Spikes.angle_difference_l, Spikes.onset]
np.save('/home/spinnaker/Documents/projects/Sevilla_2021_TDE_SSL/data/final/sweep/'+channeltype+'/' + str(frequency) + 'Hz/sweep_frequ' + str(frequency) +'_datetime_'+ str(datenow)+'.npy',data)
...
for i in range(num_channels):
	Spikes.out_r = Spikes.angle_difference_r.segments[0].spiketrains[i]
	Spikes.out_l = Spikes.angle_difference_l.segments[0].spiketrains[i]

	size_populations = [1,1]
	x_lim = [0,simtime]
	y_labels = ['out', 'out']
	spikedata_hist = [Spikes.out_r, Spikes.out_l]
